...
<section class="relative py-24 bg-[#0b1520] text-white overflow-hidden bg-cover bg-center bg-no-repeat"
    style="
    ...
    <div class="max-w-7xl mx-auto px-6 md:px-12">
        <!-- Titre -->
        <h2
            class="text-3xl md:text-4xl font-extrabold text-center
        ...
        <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-10">
            @foreach ($stories as $index => $story)
                <div x-data="{ shown: false }" x-intersect="shown = true" x-intersect:leave="shown = false"
                    ...
                    <!-- Objectif atteint -->
                    <span
                        class="inline-block px-4 py-1 mb-4 text-sm font-semibold rounded-full
                                 bg-orange-400 to-emerald-600 text-white shadow-md">
                        {{ $story['goal'] }}
                    </span>
                    ...
                    <!-- CTA -->
                    <a href="{{ $story['link'] }}"
                        class="inline-flex items-center gap-2 text-orange-400 hover:text-orange-300 font-semibold transition">
                        En savoir plus
