...
<section class="py-16 bg-black text-white overflow-hidden {{ $pauseOnHover ? 'group/marquee' : '' }}"
    @if (count($brands) > $threshold) data-marquee
        data-speed="{{ $speed }}" @endif
        ...
        @if (count($brands) > $threshold)
            ...
            <div class="relative">
                ...
                @if ($fade)
                    <!-- Masque dégradé -->
                    <div
                        class="pointer-events-none absolute inset-y-0 left-0 w-16 sm:w-24 bg-gradient-to-r from-black to-transparent">
                    </div>
                    <div
                        class="pointer-events-none absolute inset-y-0 right-0 w-16 sm:w-24 bg-gradient-to-l from-black to-transparent">
                    </div>
                @endif
            </div>
        @else
